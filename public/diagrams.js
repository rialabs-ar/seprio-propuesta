  ...
  cover: () => `
    <svg viewBox="0 0 720 280" preserveAspectRatio="xMidYMid meet">
      ...
      ${[
        ...
        {x: 360, y: 50, label: 'Comunicaciones', color: '#ef9f27', big: true, labelAbove: true},
        ...
        <g>
          ...
          ${m.big ? `<circle cx="${m.x}" cy="${m.y}" r="26" fill="none" stroke="${m.color}" stroke-width="1" opacity="0.4"><animate attributeName="r" values="26;32;26" dur="2.5s" repeatCount="indefinite"/><animate attributeName="opacity" values="0.5;0;0.5" dur="2.5s" repeatCount="indefinite"/></circle>` : ''}
          <text x="${m.x}" y="${m.labelAbove ? m.y - 38 : m.y + (m.big ? 44 : 32)}" text-anchor="middle" font-family="DM Mono" font-size="9" fill="${m.color}" letter-spacing="1.5">${m.label.toUpperCase()}</text>
